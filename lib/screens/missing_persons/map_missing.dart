...
import '../crime_alert/models/address_model.dart';
...
class _MapScreenState extends State<MapScreen> {
  ...
  void _fetchMarkers() async {
    ...
    await FirebaseFirestore.instance.collection('Missingpersons').get();


    for (QueryDocumentSnapshot document in querySnapshot.docs) {
      String address = document['residence'];
      String title1= document['name'];
      ...
      _markers.addAll(addresses.map((address) {
        ...
        return Marker(
          ...
          position: LatLng(address.latitude, address.longitude),
            infoWindow: InfoWindow(title: "Missing Person")
        );
      }).toList());
      print(_markers);
    });
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Markers Show Missing Persons'),
        centerTitle: true,
